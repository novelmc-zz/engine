        ...
        switch (args.length)
        {
            case 2:
            {
                if (args[0].equalsIgnoreCase("add"))
                {
                    ...
                    {
                        Bukkit.broadcastMessage(ChatColor.GREEN + sender.getName() + " - Readding " + player.getName() + " to the architect list");
                        ArchitectList.getImpostors().remove(player.getName());
                        ArchitectList.addIp(player, player.getAddress().getHostString());
                        return true;
                    }

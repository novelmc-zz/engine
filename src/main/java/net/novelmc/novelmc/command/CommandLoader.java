...
public class CommandLoader
{
    ...
    public static void registerCommands()
    {
        try
        {
            Pattern pattern = Pattern.compile("net/novelmc/novelmc/command/(Command_[^\\$]+)\\.class");
            CodeSource cs = NovelMC.class.getProtectionDomain().getCodeSource();
            if (cs != null)
            {
                ...
                while ((zipEntry = zip.getNextEntry()) != null)
                {
                    ...
                    if (matcher.find())
                    {
                        try
                        {
                            Class<?> commandClass = Class.forName("net.novelmc.novelmc.command." + matcher.group(1));

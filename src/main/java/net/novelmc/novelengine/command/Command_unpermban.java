    ...
    public boolean onCommand(CommandSender sender, Command cmd, String string, String[] args)
    {
        ...
        if (m.matches())
        {
            for (Ban ban : BanManager.getBansByType(BanType.PERMANENT_IP))
            {
                if (ban.getIp().equals(args[0]))
                {
                    ...
                    sender.sendMessage(NUtil.colorize("&4&lSTAFF >&r &aRemoved permanent ban for IP " + args[0]));
                    return true;
                }
            }

            sender.sendMessage(NUtil.colorize("&4&lSTAFF >&r &7That IP-address has not been permanently banned."));
            return true;
        }
        ...
        for (Ban ban : BanManager.getBansByType(BanType.PERMANENT_NAME))
        {
            if (ban.getName().equals(args[0]))
            {
                ...
                sender.sendMessage(NUtil.colorize("&4&lSTAFF >&r &aRemoved permanent ban for name " + args[0]));
                return true;
            }
        }
        sender.sendMessage(NUtil.colorize("&4&lSTAFF >&r &7That name does not appear to be be permanently banned!"));
        return true;
    }
}

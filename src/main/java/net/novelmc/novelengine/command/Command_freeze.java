...
public class Command_freeze extends CommandBase
{
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String string, String[] args)
    {
        ...
        if (NPlayer.isFrozen(player))
        {
            ...
            player.sendMessage(NUtil.colorize("&8<-> &3&lINFO &7You are no longer frozen."));
            NPlayer.frozenPlayers.remove(player);
            return true;
        }
        sender.sendMessage(NUtil.colorize("&8<-> &3&lINFO &7" + player.getName() + " is now frozen."));
        player.sendMessage(NUtil.colorize("&8<-> &3&lINFO &7You are now frozen."));
        NPlayer.frozenPlayers.add(player);
        return true;
    }
}

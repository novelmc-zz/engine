...
public class Command_freeze extends CommandBase
{
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String string, String[] args)
    {
        ...
        if (player == null)
        {
            sender.sendMessage(NUtil.colorize("&3&lINFO >&r &7That player could not be found."));
            return true;
        }

        if (NPlayer.isFrozen(player))
        {
            sender.sendMessage(NUtil.colorize("&3&lINFO >&r &7" + player.getName() + " is no longer frozen."));
            player.sendMessage(NUtil.colorize("&3&lINFO >&r &7You are no longer frozen."));
            NPlayer.frozenPlayers.remove(player);
            return true;
        }
        sender.sendMessage(NUtil.colorize("&3&lINFO >&r &7" + player.getName() + " is now frozen."));
        player.sendMessage(NUtil.colorize("&3&lINFO >&r &7You are now frozen."));
        NPlayer.frozenPlayers.add(player);
        return true;
    }
}

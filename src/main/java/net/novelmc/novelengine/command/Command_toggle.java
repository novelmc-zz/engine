    ...
    public boolean onCommand(CommandSender sender, Command cmd, String string, String[] args)
    {
        ...
        switch (args[0])
        {
            case "lavaplace":
            {
                ...
                sender.sendMessage(NUtil.colorize("&7Lava placement has been " + (NovelEngine.plugin.config.isLavaEnabled() ? "enabled." : "disabled.")));
                return true;
            }
            case "waterplace":
            {
                NovelEngine.plugin.config.setWaterEnabled(!NovelEngine.plugin.config.isWaterEnabled());
                sender.sendMessage(NUtil.colorize("&7Water placement has been " + (NovelEngine.plugin.config.isWaterEnabled() ? "enabled." : "disabled.")));
                return true;
            }
        }
        sender.sendMessage(NUtil.colorize("&7Toggleable Features"));
